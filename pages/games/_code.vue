  ...
  <div v-if="step === 'join'" class="flex w-full h-screen items-center justify-center flex-col">
    <div class="card bg-gray-800 w-10/12 sm:w-2/3 md:w-2/3 lg:w-2/4 xl:w-2/7 p-4 rounded shadow">
      ...
      <div class="flex w-full flex-col items-center">
        ...
        <input v-if="showCodeField" v-model="code" type="text" class="mt-5 w-full" placeholder="Game Code...">
        <input v-model="username" type="text" class="mt-5 w-full" placeholder="Pick a username...">
        <input v-model="password" type="text" class="mt-5 w-full" placeholder="Set a password (Optional)">
  ...
  <div class="w-full flex flex-col items-center" v-else-if="game">
    <div class="flex flex-col w-1/2">
      ...
      <div v-if="choosingSuccessor && isAdmin" class="flex w-full items-center flex-col">
        <span class="text-white font-bold text-xl text-orange-500 w-2/3 text-center">Who should be the admin once you
          leave?</span>
        <span class="text-white font-bold  text-white w-2/3 text-center">Click to choose</span>
      </div>
      
      <div class="flex flex-wrap w-full">
        <div class="flex w-1/4 " v-for="player in game.players" :key="player.username">
          <div @click="choosingSuccessor ? leaveGame(player.username) : cancelLeave"
            class="w-2/3 bg-white justify-center rounded mx-2 my-2 relative shadow"
            ...
            <template v-else>
              ...
              <h4 class="text-sm w-full text-center pb-2 blinking-text font-bold text-orange-500" v-else>Waiting...
              </h4>
            </template>
    ...
    <div class="flex flex-col w-3/4 sm:w-2/3 md:w-2/4 lg:w-2/7 xl:w-1/5">
      ...
      <!--
      <div class="w-full flex justify-left mb-5 relative">
      ...
      <div class="flex w-full justify-center items-center mt-5">
        <div v-if="game.previousCard && game.previousCard.text != ''" class="flex w-3/12 pb-2 mx-3 mb-0 flex-col">
          <div class="flex flex-col w-full items-center font-bold">
            <h4 class="text-gray-300 mb-0 text-sm">Last round:</h4>
          ...
          <div v-if="game.winingCards" class="flex flex-col">
            <card class="mt-2 zuum" v-for="card in game.winingCards" :key="card.text" :text="card.text"></card>
          </div>
        </div>
        <div class="flex w-4/6 px-4">
          <div class="flex flex-col w-full">
            ...
            <a v-show="isJudge" @click="skip" class="text-sm text-white mt-2 text-center">Skip This Card?</a>
    ...
    <div class="w-full lg:w-8/12">
      <template v-if="game.started">
        <div class="flex flex-col w-full mt-4 ">
          ...
          <div class="deck w-full nowrap px-3 relative overflow-scroll scrol-hide">
            <transition name="list" mode="out-in">
              <div key="cards" v-if="shouldShowCards" class="w-full flex-wrap justify-center">
                <div v-for="(card,index) in cards" @click="activeIndex = index"
                  ...
                  <card @play="playCard(card.text)" :active="index === activeIndex" type="white" :text="card.text">
                  </card>
              ...
              <div key="judgingCards" class="w-full flex-wrap justify-center" v-else-if="game.judgingCards.length > 0">
                ...
                <div v-for="(player,index) in game.judgingCards" :key="`player-${index}`"
                  @click="activePlayerIndex = index"
                  class="carti game-card w-1/3 sm:w-2/12 md:w-3/8 lg:w-2/8 xl:w-1/7 px-1 inline-block pb-2 relative">
                  ...
                  <card v-for="card in player" :key="card" :judging="true" @pick="pick(index)" type="white" class="mb-2"
                    :text="card"></card>
              ...
              <div key="waiting" class="w-full flex-wrap justify-center" v-else>
                <div v-for="index in game.players.length - 1"
                  class="game-card w-1/3 sm:w-2/12 md:w-3/8 lg:w-2/8 xl:w-1/7 px-1 inline-block pb-2"
                  :key="`${index}-waiting`">
      ...
      <template v-else>
        <div class="flex flex-col w-full mt-4">
          ...
          <div class="deck w-full nowrap overflow-scroll px-3 scrol-hide">
...
<script lang="ts">
  ...
  import VueSocketIOExt, {
    Socket
  } from 'vue-socket.io-extended';
  ...
  export default class GameView extends Vue {
    ...
    sendMessage() {
      ...
      this.client.post(`/api/games/${this.code}/message`, {
    ...
    setupSocket() {
      if (this.code)
        this.$socket.$subscribe(`update.${this.code}`, (payload: {
          data: GameData,
          message: string,
          sound ? : string
        }) => {
